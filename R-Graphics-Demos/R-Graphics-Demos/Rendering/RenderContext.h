...
namespace R
{
	namespace Rendering
	{
		...
		public:
			...
			inline ID3D12Resource*				GetRenderTarget(std::uint32_t index)		const	{ return m_renderTargets[index].Get(); }
			inline ID3D12DescriptorHeap*		GetRTVHeap()						const	{ return m_rtvHeap.Get(); }
			inline std::uint32_t						GetRTVDescriptorSize()				const	{ return m_rtvDescriptorSize; }
